...
class FileShell extends Shell
{
    ...
    private function sendAllStocksNotificationsInTileNotifications($Notifier, $tick_names_and_values, $allStocks)
    {
        ...
        for ($i = 0; $i < sizeof($allStocks); $i++) {
            ...
            $MyTileXML = '<tile><visual version="2"><binding template="TileSquare150x150Text01" fallback="TileSquareText01"><text id="1">'.$tick_name.' (larger text)</text><text id="2">'.$value.'</text></binding></visual></tile>';
            /*$MyTileXML = '<tile>
  <visual>
